...
const { creatingUserData } = global.DB.database;
...
module.exports = async function (databaseType, userModel, api, fakeGraphql) {
      ...
      return new Promise((resolve, reject) => {
          messageQueue.push(async function () {
              try {
                  ...
                  const points = await get_(userID, "points");
                  resolve(points);
              }
              catch (err) {
                  reject(err);
  ...
  async function addPoints(userID, points, query) {
      return new Promise((resolve, reject) => {
          messageQueue.push(async function () {
              try {
                  if (isNaN(userID)) {
                  ...
                  if (isNaN(points)) {
                      throw new CustomError({
                          name: "INVALID_POINTS",
                          message: `The second argument (points) must be a number, not ${typeof points}`
                      });
                  }
                  if (!global.db.allUserData.some(u => u.userID == userID))
                      await create_(userID);
                  const currentPoints = await get_(userID, "points");
                  const newPoints = currentPoints + points;
                  const userData = await save(userID, newPoints, "update", "points");
                  if (query)
                      if (typeof query !== "string")
                          throw new Error(`The third argument (query) must be a string, not ${typeof query}`);
                      else
  ...
  async function subtractPoints(userID, points, query) {
      return new Promise((resolve, reject) => {
          messageQueue.push(async function () {
              try {
                  if (isNaN(userID)) {
